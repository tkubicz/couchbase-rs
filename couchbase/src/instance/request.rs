    ...
    fn encode(self: Box<Self>, instance: *mut lcb_INSTANCE) {
        ...
        unsafe {
            ...
            for spec in self.specs {
                ...
                let path_encoded = CString::new(spec.path().clone()).expect("Could not encode path");

                match spec.command_type() {
                    SubdocLookupCommandType::Get => {
                        lcb_subdocops_get(ops, idx, flags, path_encoded.as_ptr(), path_len);
                    },
                    SubdocLookupCommandType::Count => {
                        lcb_subdocops_get_count(ops, idx, flags, path_encoded.as_ptr(), path_len);
                    },
                    SubdocLookupCommandType::Exists => {
                        lcb_subdocops_exists(ops, idx, flags, path_encoded.as_ptr(), path_len);
                    },
